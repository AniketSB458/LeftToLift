...
import { useGetImpactCounters, useGetAllDonations, useGetAllNgos } from '../hooks/useQueries';
...
export default function ImpactDashboard() {
  const { t } = useLanguage();
  const { data: impact, isLoading: impactLoading } = useGetImpactCounters();
  ...
  const weeklyData = [
    ...
    { day: 'Fri', meals: Math.floor(totalMeals * 0.20) },
    { day: 'Sat', meals: Math.floor(totalMeals * 0.11) },
    { day: 'Sun', meals: Math.floor(totalMeals * 0.10) },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Real-time metrics on food rescued and lives impacted across Maharashtra
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { icon: <Utensils className="h-6 w-6" />, label: 'Total Meals Saved', value: totalMeals.toLocaleString('en-IN'), color: 'text-amber-600' },
          { icon: <Users className="h-6 w-6" />, label: 'People Fed', value: totalPeople.toLocaleString('en-IN'), color: 'text-blue-600' },
          { icon: <Leaf className="h-6 w-6" />, label: 'CO₂ Reduced (kg)', value: Math.round(co2Reduced).toLocaleString('en-IN'), color: 'text-green-600' },
          { icon: <MapPin className="h-6 w-6" />, label: 'Cities Active', value: cityBreakdown.length.toString(), color: 'text-purple-600' },
        ].map((card, i) => (
          <Card key={i}>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Total meals rescued per Maharashtra city</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={cityChartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                <XAxis dataKey="city" tick={{ fontSize: 12 }} />
                <YAxis tick={{ fontSize: 12 }} />
                <Tooltip formatter={(v) => [Number(v).toLocaleString('en-IN'), 'Meals']} />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Current status of all submitted donations</CardDescription>
          </CardHeader>
          <CardContent>
            {donutData.length > 0 ? (
              <ResponsiveContainer width="100%" height={220}>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Estimated daily meal distribution this week</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={weeklyData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                ...
                <YAxis tick={{ fontSize: 12 }} />
                <Tooltip formatter={(v) => [Number(v).toLocaleString('en-IN'), 'Meals']} />
                <Line type="monotone" dataKey="meals" stroke="#10b981" strokeWidth={2} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
                ...
                {leaderboard.map(([name, count], i) => (
                  <div key={name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">{count} donation{count > 1 ? 's' : ''}</p>
                    </div>
                    {i === 0 && <Badge variant="secondary" className="text-xs">🏆 Top Donor</Badge>}
                  </div>
                ))}
              </div>
            ) : (
