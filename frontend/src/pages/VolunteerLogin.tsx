...
import { Users, Loader2, ArrowLeft, Bike, Clock, Award } from 'lucide-react';
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          <div className="bg-card border border-blue-200 dark:border-blue-800 rounded-3xl shadow-xl overflow-hidden">
            ...
            <div className="p-8">
              <div className="flex flex-col items-center text-center mb-8">
                ...
                <h1 className="text-2xl font-extrabold text-foreground">Volunteer Login</h1>
                <p className="text-muted-foreground mt-2 text-sm">
                  Pick up and deliver food donations to NGOs and communities across Maharashtra.
                </p>
              </div>

              <div className="space-y-3 mb-8">
                {[
                  { icon: Bike, text: 'Claim food pickup assignments near you' },
                  { icon: Clock, text: 'Flexible scheduling — work on your time' },
                  { icon: Award, text: 'Earn recognition for your contributions' },
                ].map(({ icon: Icon, text }) => (
              ...
              {!isAuthenticated ? (
                ...
                <form onSubmit={handleProfileSetup} className="space-y-4">
                  ...
                  <button type="button" onClick={handleLogout} className="w-full text-sm text-muted-foreground hover:text-foreground transition-colors">
                    Cancel & Logout
                  </button>
                </form>
              ) : (
          ...
          <p className="text-center text-xs text-muted-foreground mt-6">
            Secured by <span className="font-semibold text-foreground">Internet Identity</span> — no passwords needed.
          </p>
        </div>
      </main>
    </div>
