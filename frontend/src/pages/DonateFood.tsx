...
import { Mic, MicOff, MapPin, CheckCircle, AlertTriangle, XCircle, Star, Loader2 } from 'lucide-react';
import { toast } from 'sonner';
import { useLanguage, useEmergencyMode } from '../App';
import { useCreateFoodDonation, useGetNgosByCity, useRateNgo } from '../hooks/useQueries';
import { computeFoodSafety, type FoodSafetyResult } from '../utils/foodSafetyEngine';
import { matchNgos, type NgoMatch } from '../utils/ngoMatcher';
import { Skeleton } from '@/components/ui/skeleton';

const MAHARASHTRA_CITIES = ['Mumbai', 'Pune', 'Nagpur', 'Nashik', 'Aurangabad', 'Kolhapur'];

const CITY_NEIGHBORHOODS: Record<string, string[]> = {
  Mumbai: ['Andheri East', 'Andheri West', 'Dadar West', 'Bandra Kurla Complex', 'Bandra West', 'Borivali', 'Malad', 'Goregaon', 'Kurla', 'Chembur'],
  Pune: ['FC Road', 'Hinjewadi', 'Kothrud', 'Shivajinagar', 'Koregaon Park', 'Viman Nagar', 'Hadapsar', 'Wakad', 'Baner', 'Aundh'],
  Nagpur: ['Sitabuldi', 'Wardha Road', 'Dharampeth', 'Sadar', 'Ramdaspeth', 'Manish Nagar', 'Hingna', 'Kamptee'],
  Nashik: ['Gangapur Road', 'College Road', 'Panchavati', 'Nashik Road', 'Cidco', 'Satpur'],
  Aurangabad: ['Cidco', 'Garkheda', 'Osmanpura', 'Cantonment', 'Waluj', 'Aurangpura'],
  Kolhapur: ['Rajarampuri', 'Shivaji Park', 'Tarabai Park', 'Kasba Bawada', 'Shahupuri'],
};
...
const FOOD_TYPES = [
  'Vegetable Curry', 'Dal', 'Rice', 'Roti', 'Chapati', 'Biryani',
  'Khichdi', 'Sabzi', 'Poha', 'Upma', 'Idli', 'Dosa', 'Sambar',
  'Bread', 'Fruits', 'Salad', 'Sweets', 'Mithai', 'Snacks',
];
...
function SafetyBadge({ result }: { result: FoodSafetyResult }) {
  ...
  return (
    <div className={`flex items-center gap-3 p-4 rounded-xl border-2 ${colors[result.status]} transition-all duration-500`}>
      {icons[result.status]}
      <div>
...
export default function DonateFood() {
  ...
  // Result state
  const [safetyResult, setSafetyResult] = useState<FoodSafetyResult | null>(null);
  const [ngoMatches, setNgoMatches] = useState<NgoMatch[]>([]);
  const [submittedDonationId, setSubmittedDonationId] = useState<bigint | null>(null);
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      const found = FOOD_TYPES.find(f => transcript.toLowerCase().includes(f.toLowerCase()));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const safety = computeFoodSafety(foodType, cookTimeMs * 1_000_000, storageCondition, city);
    setSafetyResult(safety);

    if (ngosInCity && ngosInCity.length > 0) {
      const matches = matchNgos(donorCoords.lat, donorCoords.lon, ngosInCity, safety.remainingHours);
      ...
      <div className="min-h-screen bg-background py-10 px-4">
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="text-center">
            ...
            <h1 className="text-3xl font-black text-foreground mb-2">Donation Submitted!</h1>
            <p className="text-muted-foreground">Your food donation is now being matched with the nearest NGO.</p>
          </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6">
              ...
              <div className="bg-primary/5 rounded-xl p-4 border border-primary/20 mb-4">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-bold text-foreground text-lg">{topNgo.ngo.name}</h3>
                    <p className="text-muted-foreground text-sm">{topNgo.ngo.neighborhood}, {topNgo.ngo.city}</p>
                    <p className="text-sm text-primary font-medium mt-1">
                      📍 {topNgo.distanceKm.toFixed(1)} km · ⏱ ~{topNgo.estimatedPickupMinutes} min
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="text-yellow-500 font-bold">⭐ {topNgo.ngo.rating.toFixed(1)}</div>
                    <div className="text-xs text-muted-foreground">Capacity: {topNgo.ngo.capacity.toString()}</div>
                  </div>
                </div>
              </div>
              ...
              {ratedNgoId !== topNgo.ngo.id && (
                <div>
                  <p className="text-sm font-semibold text-foreground mb-2">Rate this NGO:</p>
                  <StarRating onRate={(r) => handleRateNgo(topNgo.ngo.id, r)} />
                </div>
              )}
              {ratedNgoId === topNgo.ngo.id && (
                <p className="text-sm text-green-600 font-medium">✅ Thank you for your rating!</p>
              )}
            </div>
          )}
          ...
          {ngoMatches.length > 1 && (
            <div className="bg-card border border-border rounded-2xl p-6">
              <h2 className="font-bold text-foreground mb-3">🗺️ Other Nearby NGOs</h2>
              <div className="space-y-2">
                {ngoMatches.slice(1).map(match => (
                  <div key={match.ngo.id.toString()} className="flex items-center justify-between p-3 bg-muted/50 rounded-xl">
                    <div>
                      <p className="font-medium text-foreground text-sm">{match.ngo.name}</p>
                      <p className="text-xs text-muted-foreground">{match.ngo.city}</p>
                    </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={() => { setStep('form'); setSafetyResult(null); setNgoMatches([]); }}
  ...
  return (
    <div className={`min-h-screen py-10 px-4 ${emergencyMode ? 'bg-red-50 dark:bg-red-950/20' : 'bg-background'}`}>
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-card border border-border rounded-2xl p-6 space-y-5">
          {/* Donor Name */}
          <div>
            <label className="block text-sm font-semibold text-foreground mb-1.5">Your Name / Hotel Name *</label>
            <input
              type="text"
              value={donorName}
          ...
          {/* Food Type */}
          <div>
            <label className="block text-sm font-semibold text-foreground mb-1.5">{t('donate.foodType')} *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-semibold text-foreground mb-1.5">{t('donate.quantity')} *</label>
              <input
                type="number"
                value={quantity}
            ...
            <div>
              <label className="block text-sm font-semibold text-foreground mb-1.5">{t('donate.unit')}</label>
              <select
                value={unit}
              ...
              >
                {['servings', 'kg', 'litres', 'packets', 'boxes', 'plates'].map(u => (
          ...
          {/* Cook Time */}
          <div>
            <label className="block text-sm font-semibold text-foreground mb-1.5">{t('donate.cookTime')} *</label>
            <input
              type="datetime-local"
              value={cookTime}
          ...
          {/* Storage Condition */}
          <div>
            <label className="block text-sm font-semibold text-foreground mb-1.5">{t('donate.storage')}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-semibold text-foreground mb-1.5">{t('donate.city')}</label>
              <select
                value={city}
                onChange={e => { setCity(e.target.value); setNeighborhood(CITY_NEIGHBORHOODS[e.target.value]?.[0] ?? ''); }}
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
              >
            ...
            <div>
              <label className="block text-sm font-semibold text-foreground mb-1.5">{t('donate.neighborhood')}</label>
              <select
                value={neighborhood}
                onChange={e => setNeighborhood(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
              >
                {(CITY_NEIGHBORHOODS[city] ?? []).map(n => <option key={n} value={n}>{n}</option>)}
              </select>
            </div>
          </div>
          ...
          <div>
            ...
            >
              ...
              {useCurrentLocation ? '✅ Using current location' : 'Use my current location (optional)'}
            </button>
          </div>
