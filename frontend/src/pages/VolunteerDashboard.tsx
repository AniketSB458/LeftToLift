...
import { MapPin, Clock, Truck, Loader2, AlertTriangle } from 'lucide-react';
import { toast } from 'sonner';
import { useLanguage, useEmergencyMode } from '../App';
import { useGetAvailableDonationsForVolunteers, useGetAllVolunteers, useClaimDonation } from '../hooks/useQueries';
...
function CountdownTimer({ expiryTime, status }: { expiryTime: Date; status: string }) {
...
function AssignmentCard({
  ...
  const distance = haversineDistance(VOLUNTEER_LOCATION.lat, VOLUNTEER_LOCATION.lon, donation.latitude, donation.longitude);
  ...
  return (
    <div className={`bg-card border-2 rounded-2xl p-5 hover:shadow-md transition-all duration-300 ${emergency ? 'border-red-500' : urgencyColors[safety.status]}`}>
      ...
      <div className={`p-3 rounded-xl mb-3 text-sm font-medium ${
        emergency || safety.status === 'unsafe' ? 'bg-red-100 text-red-700' :
        safety.status === 'urgent' ? 'bg-yellow-100 text-yellow-700' :
      ...
      <div className="grid grid-cols-2 gap-2 text-sm mb-3">
        <div className="text-muted-foreground">🍽️ {donation.quantity} {donation.unit}</div>
        <div className="text-primary font-medium">📍 {distance.toFixed(1)} km away</div>
        <div className="text-muted-foreground">🏠 {donation.storageCondition}</div>
        <div className="text-orange-600 font-medium">⏱ ~{pickupMin} min ETA</div>
      </div>
      ...
      >
        {isClaiming ? <Loader2 className="w-4 h-4 animate-spin" /> : <Truck className="w-4 h-4" />}
...
export default function VolunteerDashboard() {
  ...
  const [selectedVolunteerId, setSelectedVolunteerId] = useState<bigint | null>(null);
  const [claimingId, setClaimingId] = useState<bigint | null>(null);

  const { data: donations, isLoading: donationsLoading } = useGetAvailableDonationsForVolunteers();
  const { data: volunteers, isLoading: volunteersLoading } = useGetAllVolunteers();
  ...
  const sortedDonations = [...(donations ?? [])].sort((a, b) => {
    ...
    const safetyA = computeFoodSafety(a.foodType, Number(a.cookTime), a.storageCondition, a.city);
    const safetyB = computeFoodSafety(b.foodType, Number(b.cookTime), b.storageCondition, b.city);
  ...
  const handleClaim = async (donation: FoodDonation) => {
    ...
    try {
      await claimMutation.mutateAsync({ donationId: donation.id, volunteerId: volId });
    ...
    <div className={`min-h-screen py-10 px-4 ${emergencyMode ? 'bg-red-50 dark:bg-red-950/20' : 'bg-background'}`}>
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-black text-foreground">🚚 {t('volunteer.title')}</h1>
            <p className="text-muted-foreground mt-1">Available food pickups near you</p>
          </div>
          {emergencyMode && (
        ...
        {!isLoading && volunteers && volunteers.length > 0 && (
          <div className="bg-card border border-border rounded-2xl p-4 mb-6">
            <label className="block text-sm font-semibold text-foreground mb-2">Select Your Volunteer Profile:</label>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 text-muted-foreground">
            ...
            <p className="text-xl font-medium">No pickups available right now</p>
            <p className="text-sm mt-2">Check back soon — new donations are added regularly</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {sortedDonations.map(donation => {
              const assignedVolunteer = donation.claimedBy != null
                ? volunteers?.find(v => v.id === donation.claimedBy) ?? null
                : null;
              return (
                <AssignmentCard
