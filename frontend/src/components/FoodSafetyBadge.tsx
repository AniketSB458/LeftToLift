...
import { FoodSafetyResult } from '../utils/foodSafetyEngine';
...
export function FoodSafetyBadge({ result, compact = false }: FoodSafetyBadgeProps) {
  const icons = {
  ...
  return (
    <div className={`rounded-xl p-4 border ${classes[result.status]}`}>
      <div className="flex items-center gap-2 mb-2">
        {icons[result.status]}
        <span className="font-bold text-sm uppercase tracking-wide">{labels[result.status]}</span>
      </div>
