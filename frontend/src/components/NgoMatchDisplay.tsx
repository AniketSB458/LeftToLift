...
import { NgoMatch } from '../utils/ngoMatcher';
    ...
    <div className="space-y-3 animate-fade-in-up">
      ...
      <div className="rounded-2xl p-4 border-2 border-brand-green bg-green-50 dark:bg-green-900/20">
        <div className="flex items-start justify-between mb-2">
          <div>
            <span className="text-xs font-bold text-brand-green uppercase tracking-wide">Best Match</span>
            <h3 className="font-bold text-foreground text-lg">{best.ngo.name}</h3>
          ...
          <div className="flex items-center gap-1 text-yellow-500">
            ...
            <span className="text-sm font-semibold">{Number(best.ngo.rating).toFixed(1)}</span>
          </div>
        </div>
        <div className="flex gap-4 text-sm">
          <div className="flex items-center gap-1 text-muted-foreground">
            <MapPin className="w-3.5 h-3.5 text-brand-green" />
            <span className="font-medium">{best.distanceKm.toFixed(1)} km away</span>
          </div>
          <div className="flex items-center gap-1 text-muted-foreground">
            <Clock className="w-3.5 h-3.5 text-brand-orange" />
            <span className="font-medium">~{best.estimatedPickupMinutes} min pickup</span>
          </div>
        </div>
        <p className="text-xs text-brand-green font-medium mt-2">
          Nearest trust found {best.distanceKm.toFixed(1)} km away – Estimated pickup in{' '}
          {best.estimatedPickupMinutes} minutes
        </p>
      </div>
      ...
      {rest.map((match, i) => (
        ...
        >
          <div>
            ...
            <p className="text-xs text-muted-foreground">{match.ngo.neighborhood}</p>
          </div>
