...
import { FoodDonation } from '../backend';
...
export function DonationCard({
  ...
  const safety = computeFoodSafety(donation.foodType, Number(donation.cookTime), storageKey, donation.city);
  ...
  const priorityLabel =
    safety.status === 'unsafe' ? 'Critical' : safety.status === 'urgent' ? 'High' : 'Normal';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${priorityClass}`}>
              {priorityLabel}
            </span>
          </div>
      ...
      <div className="mb-3">
        <p className="text-xs font-medium text-muted-foreground">{safety.message}</p>
      </div>
      ...
      {donation.status !== 'pending' && (
        <div className={`text-xs font-semibold capitalize ${statusColors[donation.status] ?? ''}`}>
