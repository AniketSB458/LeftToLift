...
import { VolunteerMatch } from '../utils/volunteerMatcher';
import { User, MapPin, Star, Clock, Bell } from 'lucide-react';
...
export function VolunteerAssignmentCard({
  ...
  const urgencyText = remainingHours < 1
  ...
  return (
    <div className="rounded-2xl p-4 border-2 border-brand-orange bg-orange-50 dark:bg-orange-900/20 animate-scale-in">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs font-bold text-brand-orange uppercase tracking-wide">Volunteer Assigned</span>
      </div>
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{match.volunteer.availability}</p>
          </div>
        </div>
        <div className="flex items-center gap-1 text-yellow-500">
          <Star className="w-4 h-4 fill-current" />
          <span className="text-sm font-semibold">{Number(match.volunteer.rating).toFixed(1)}</span>
        </div>
      </div>
      ...
      <div className="rounded-lg p-3 bg-orange-100 dark:bg-orange-900/30 text-sm">
        <p className="font-medium text-orange-800 dark:text-orange-300">
          "Urgent food pickup near {neighborhood}. {quantity} {unit} of {foodType} expiring in {urgencyText}."
        </p>
      </div>
    </div>
  );
