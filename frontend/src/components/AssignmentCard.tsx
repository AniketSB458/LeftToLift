...
import { MapPin, Navigation, Clock, Package, CheckCircle, Truck } from 'lucide-react';
...
export function AssignmentCard({
  ...
  const { remainingText, urgencyBorderClass, urgencyLevel } = useCountdownTimer(expiryMs, totalDurationMs);
  ...
  return (
    <div className={`glass-card rounded-2xl p-5 ${urgencyBorderClass} ${urgencyBg} transition-all duration-300`}>
      ...
      <div className="space-y-2 mb-4">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-muted-foreground">
            {distanceKm.toFixed(1)} km · ~{Math.round((distanceKm / 30) * 60)} min
          </span>
        </div>
      </div>
