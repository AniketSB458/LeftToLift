...
export function useUpdateDonationStatus() {
  ...
  return useMutation({
    mutationFn: async ({ id, status }: { id: bigint; status: DonationStatus }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useClaimDonation() {
  ...
  return useMutation({
    mutationFn: async ({ donationId, volunteerId }: { donationId: bigint; volunteerId: bigint }) => {
      if (!actor) throw new Error('Actor not available');
      await actor.claimDonation(donationId, volunteerId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['availableDonationsForVolunteers'] });
      queryClient.invalidateQueries({ queryKey: ['allDonations'] });
    },
    onError: (error: Error) => {
...
export function useRateNgo() {
  ...
  return useMutation({
    mutationFn: async ({ ngoId, rating }: { ngoId: bigint; rating: number }) => {
      if (!actor) throw new Error('Actor not available');
