...
export function computeFoodSafety(
  ...
  const expiryTime = new Date(cookTimeDate.getTime() + totalShelfLife * 60 * 60 * 1000);
  ...
  if (remainingHours >= 3) {
    ...
    status = 'unsafe';
    message = remainingHours <= 0 ? 'Food may have expired' : `Critical — only ${(remainingHours * 60).toFixed(0)} minutes remaining`;
  }
