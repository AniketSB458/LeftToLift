...
export function optimizeRoute(
  ...
  const sorted = [...donations].sort((a, b) => Number(a.cookTime) - Number(b.cookTime));
  ...
  return sorted.map((donation, index) => {
    const dist = haversineDistance(currentLat, currentLon, donation.latitude, donation.longitude);
